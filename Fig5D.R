...
sink( 'Rout/Fig5D.Rout' )

phens	<- c( "asthma_onset_frombirth", "Pre.FEV1.perc.pred", "Pre.FEV1.FVC.perc.pred", "deltafev", "tIGE" ) 
ylabs	<- c( 'Age-of-Onset Heritability', '', 'FEV1/FVC Heritability', '', '' ) 
type	<- 'ctr'
suff	<- ifelse( type == 'imp', '', paste0('_',type) )
qn		<- FALSE ### was already quantnormed, so TRUE is pointless/identical
...
for( phen in phens ){
	load( paste0( 'Rdata/gxemm/', phen, suff, '_qn=', qn, '.Rdata' ) ) 
	...
	print( phen ); print( out$fits$free ); print( ps[phen,'free_lr'] )

	for( k in 1:1000 ){
		load( paste0( 'Rdata/gxemm/', phen, '_rand', k, suff, '_qn=', qn, '.Rdata' ) )
...
for( phen in c( "Pre.FEV1.FVC.perc.pred", "asthma_onset_frombirth" ) ){
	...
	pdf( paste0( 'figs/Fig5D_', phen, '.pdf' ), width=3.2, height=5.5 )
	par( mar=c(.5,4.5,.5,1) )
