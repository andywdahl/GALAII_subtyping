...
K				<- 3
locphens<- c( 'mlr', 'tIGE', 'asthma_onset_frombirth', 'Pre.FEV1.perc.pred', 'Pre.FEV1.FVC.perc.pred', 'deltafev' )#, "hosp_er_12mo" )
ylabs		<- c( 'Asthma (Log OR)', 'IgE', 'Age of Onset', 'FEV1', 'FEV1/FVC', 'Albuterol\nResponse' )#, 'Exacerbation (Log OR)' )
names(ylabs)<- locphens
np	<- length(locphens)


plotdata	<- 'Rdata/Fig4.Rdata'
if( file.exists( plotdata ) ){
	load( plotdata )
} else { 
	goodrs		<- unique(c(
		'rs2033784', 'rs992969', 'rs2544523',	# asthma
		'rs12568083', 'rs4129267',
		'rs13194312',
		'rs7593948', 'rs666353',
		'rs2396255', 'rs77941914',
		'rs114204740', 'rs7593948'
	)) 
	print( round( cbind( mafs[goodrs], miss[goodrs] ), 2 ) )
	goodrs	<- goodrs[mafs[goodrs]>.05] 
	...
	#pvs		[p,,1	]	<- sapply( out, function(x) 10^-x$log10p_hom )
	...
	for( p in locphens[-1] ){
		...
		#pvs		[p,,'hom'	]	<- sapply( out, function(x) x$homp )
...
pdf( 'figs/Fig5AB.pdf', width=9.4, height=5.0 )
layout( matrix( 1:6, 1, 6, byrow=T ), widths=c(1.4,6.5,2.5,2.5,2.5,4.0) )
par( mar=rep(0,4) )
par( mar=c(14.5,0,4,0) )
plot.new()
mtext(2,line=-4.1,text='SNP Effects on Asthma\nand Related Traits', cex=1.25)

#snptypes	<- c( 'Known Asthma\nGWAS Hit', 'Known Asthma\nGWAS Hit', 'Known Asthma\nGWAS Hit', 'Hom Effect\n on IgE', 'Known Asthma\nGWAS Hit', 'Hom Effect on\nAge of Onset', 'Hom Effect\n on Asthma', 'Hom Effect\n on  Asthma', 'Hom Effect\n on  Asthma', 'Hom Effect on\nAlb. Response' )
snptypes	<- c( 'Yes', 'Yes', 'Yes', 'No', 'Yes', 'No', 'No', 'No', 'No', 'No' )
names(snptypes)	<- goodrs
par( mar=c(14.5,2.5,4,1.9) )
for( p in locphens[c(1,3,6,5,2)] ){
	if( p == locphens[1] ){
	...
	#	is	<- c( 'rs7593948', 'rs666353' )
	} else if( p == locphens[5] ){
		is	<- c( 'rs2396255' )
	} else if( p == locphens[6] ){
		is	<- c( 'rs7593948' )
	}
	...
	points(xs, ys, pch=16, cex=4																, col=rep(cols,length(is)) )
	arrows(xs, ys-yse, xs, ys+yse, length=0.09, angle=90, code=3, col=rep(cols,length(is)), lwd=2.5 )
	rm(is)
}
